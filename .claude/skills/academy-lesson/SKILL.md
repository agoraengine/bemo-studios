...
1. **Generate.** One HeyGen render per movement from the locked script text, using the lesson's category presenter (`avatar_id` + matched voice from the brief's cast table): `create_video_from_avatar` for singles, `create_video_batch` + `bulk_video_statuses` polling for batches. Generation text rules (treatment.md, per Lee 2026-08-04): full-screen movements render with a landscape background and `fit: cover` (no pillarboxing); phonetic spellings for brand words (BeMo first) and `[pause]` directives go into the text sent to HeyGen only, while script.md keeps real spellings and captions burn from the script. These public studio looks need `engine: avatar_iii`; the twins take `avatar_v`. Download renders to `productions/academy-lessons/capture/out/<slug>/avatar/` (gitignored). VO matches the locked script word for word; if a read demands a change, change the script first (it drops back to Scripted for re-ratification). Roster → Generated.

...
4. **Audition before pilot, pilot before batches.** Both presenter seats are settled by the audition round in the brief: one short sample passage rendered across a few twin looks and a few stock avatars, reviewed by Becky as finished renders side by side (never described to her in place of rendering). Until she ratifies one twin look and one generic presenter into the decision table, nothing pilots. Then, until one lesson has Shipped status, exactly one lesson (Becky's pick) moves at a time; batch mode opens only after she approves the finished pilot. Each presenter pilots separately: the first lesson using the other presenter also runs alone before batches open for its product areas.

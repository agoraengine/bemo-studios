...
> **Status: legacy (Becky, 2026-08-03).** Common Table is now the standing universe for every demo and website capture (see `../README.md`). Wrenfield exists only to serve super-demo-60 if the Aug 9 cut needs a recapture, and retires when that production closes. Do not load it or use it for anything new.
...
> Read `demo-org/wrenfield/fact-sheet.md`. Draft [document] for The Wrenfield Alliance consistent with every fact on that sheet. Fictitious names only; web-search any new proper noun for collisions and add it to the fact sheet. No em dashes anywhere. Avoid the banned AI-tell words in `../bemo-os/CLAUDE.md`. Write like a sharp nonprofit staffer, not a brochure.

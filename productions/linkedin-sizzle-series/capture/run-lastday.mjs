#!/usr/bin/env node
// Renders Reel 1D, "The Last Day" (succession storyline, commercial grammar).
//
//   node run-lastday.mjs --build      splice images into source-lastday.built.html
//   node run-lastday.mjs --render     record 2560x1440
//   node run-lastday.mjs --finish     encode + VO + music + captions  (--outtag vC1)
//
...
const SLUG = "bemo-linkedin-sizzle-series-r1d";
const DUR = 58, TAIL = 2;
const W = 2560, H = 1440;
...
const MIX = [
  ...
  [path.join(VO, "hgd-n1.wav"), 4.8],
  [path.join(VO, "hgd-n2.wav"), 6.7],
  [path.join(VO, "hgd-n4.wav"), 9.5],
  [path.join(VO, "hgd-n5.wav"), 15.55],
  [path.join(VO, "hgc-c6.wav"), 17.1],
  [path.join(VO, "hgd-n7.wav"), 19.9],
  [path.join(VO, "hg-r1-fourapps.wav"), 21.2],
  [path.join(VO, "hgc-c10.wav"), 25.8],
  [path.join(VO, "hgc-c11.wav"), 27.1],
  [path.join(VO, "hgc-c12.wav"), 33.6],
  [path.join(VO, "hgc-c13.wav"), 38.0],
  [path.join(VO, "hge-nu.wav"), 42.45],
  [path.join(VO, "hgd-n14.wav"), 46.8],
  [path.join(VO, "hg-r1-l8.wav"), 49.1],
  [path.join(VO, "hgc-c14.wav"), 53.5],
];
...
const SFX = [
  [SFX_CHIME, 15.4, 0.0, 1.4, 0.12],
];
const SWELL_AT = 46.6;

function build() {
  const tpl = fs.readFileSync(path.join(HERE, "source-lastday.html"), "utf8");
  const b64 = (f, mime) =>
  ...
  fs.writeFileSync(path.join(HERE, "source-lastday.built.html"), html);
  console.log("source-lastday.built.html built.");
}
...
async function render() {
  ...
  await page.goto("file://" + path.join(HERE, "source-lastday.built.html"));

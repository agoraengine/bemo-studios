#!/usr/bin/env node
// Renders Reel 1A, the app cut: the website's punch, real app footage.
//
//   node run-appcut.mjs --build      splice images into source-appcut.built.html
//   node run-appcut.mjs --render     record 2560x1440
//   node run-appcut.mjs --finish     encode + VO + music + captions  (--outtag vC1)
//
...
const SLUG = "bemo-linkedin-sizzle-series-r1a";
const DUR = 58, TAIL = 2;
const W = 2560, H = 1440;
...
const MIX = [
  ...
  [path.join(VO, "hgw-w3.wav"), 4.6],
  [path.join(VO, "hgw-w4.wav"), 10.5],
  [path.join(VO, "hgw-w5.wav"), 16.0],
  [path.join(VO, "hgw-w6.wav"), 18.8],
  [path.join(VO, "hgw-w7.wav"), 23.0],
  [path.join(VO, "hgw-w8.wav"), 27.4],
  [path.join(VO, "hgw-w10.wav"), 35.4],
  [path.join(VO, "hgc-c12.wav"), 39.8],
  [path.join(VO, "hgc-c13.wav"), 44.0],
  [path.join(VO, "hgw-w11.wav"), 48.5],
  [path.join(VO, "hgw-w12.wav"), 52.5],
  [path.join(VO, "hgw-w13t.wav"), 54.6],
];
...
const SFX = [
  [SFX_CHIME, 22.8, 0.0, 1.4, 0.10],
];
const SWELL_AT = 52.3;

function build() {
  const tpl = fs.readFileSync(path.join(HERE, "source-appcut.html"), "utf8");
  const b64 = (f, mime) =>
  ...
  fs.writeFileSync(path.join(HERE, "source-appcut.built.html"), html);
  console.log("source-appcut.built.html built.");
}
...
async function render() {
  ...
  await page.goto("file://" + path.join(HERE, "source-appcut.built.html"));

...
> **Numbering note (2026-08-11):** the reel numbers in this file's headers are frozen production codes from before the renumber, kept because shot IDs, VO filenames, and render logs are built on them. The series numbers people and boards use are publication order, mapped in `roster.md` (the naming authority). Quick keys: "Reel 8 / 8H" here is series reel 1 and its 1H hero; "Reel 1 / 1R" here is series reel 5.
...
| 0:09 | This year she built it in BeMo, in her organization's own voice. It took about an hour. (Becky's line edit, 2026-08-16: "it took about an hour" replaces the draft's clipped "About an hour"; the hedge stays per the claim rules.) | The real Amplify annual-report flow (existing capture), provenance: "Captured from the product". | R4-S2 |
| 0:16 | (silent, music only) | Quote card: "What followed was the most painless report process I have ever experienced." Jennifer Allen, Executive Director, Friends of the Saratoga Springs Public Library. BeMo beta. (Corrected 2026-08-16: the draft's card trimmed "What followed was" from her sentence; quotes run as written, so the card carries the full sentence from her story file.) | R4-S3 |
...
### Reel 8H: the 60-second hero cut (FINAL: vH18 approved by Becky, 2026-08-09)
...
|---|---|---|
| R4-v3 | 2026-08-16 | Becky's line edit on the Sunday review: line 2 closes "It took about an hour." instead of the clipped "About an hour." Fresh read (`hg-r4-l2c.wav`, one-word Beemo spelling, 5.36s; the name renders tight at 0.34s), caption and srt updated, everything else untouched. QA: 30.0s, -16.2 LUFS, -1.1 dBTP. PENDING Becky on the batch review page; v2 superseded |
| R4-v2 | 2026-08-16 | Becky's note on v1: the word BeMo is off and needs a recut. Fresh reads of line 2 and the close on her twin voice with the generation text carrying **"Beemo" as one word** instead of the standing "Bee Mo" phonetic: on the audio-only speech endpoint the two-word phonetic splays the name into two separated tokens (timestamps showed a 0.18s gap and a drawn second syllable in the close), while the one-word spelling renders it as one tight unit (0.32s in line 2, 0.38s in the close, natural sentence pause after). The close also drops the SSML break for a plain-text read whose natural pause lands 0.66s. IPA-phoneme alternates generated and archived (`hg-r4-l2-ipa/l5-ipa.wav`, slower reads) in case v2 still reads off; v1 reads archived as `hg-r4-l2/l5.wav`. Placements re-derived (line 2 ends 10.5, close 23.6 to 26.7); captions and srt updated. QA: 30.0s, -16.4 LUFS, -0.7 dBTP. Worth recording for the phonetic ledger: the "Bee Mo" phonetic was proven on the Avatar V path; the speech endpoint appears to want the one-word spelling. PENDING Becky, on the Sunday batch review page |
| R4-v1 | 2026-08-16 | First render of series reel 2, An Hour Not Three Days (production code R4), for the Monday Aug 17 company-page slot; the Week 30 pull found the render queue had stopped after reel 1. The pull record's note that Becky's VO existed for this reel was wrong: `vo/becky/r2-pass-a/b` are a second recording session of reel 1's lines (the pass-b transcript announces "Line one, Line two" matching the reel 1 sheet), so this cut carries four fresh speech-endpoint reads on her twin voice (`hg-r4-l{1,2,4,5}.wav`; Bee Mo phonetic, 1.12x, en-US, SSML break for the close beat; one pipeline end to end per the vH12 rule). Five scenes per the script: open card (headline broken before "about an hour"), the interview beat from the annual-report capture (ss 92, the editor asking the questions with the facts rail filling; gentle 1.12 to 1.32 push; provenance label on), the quote card silent with her full verbatim sentence (see the corrected R4-S3 row), the Amplify lead-message card, the standing close (white knockout on Deep Sapphire). Voiced-cut mix standard: static measured gains, sidechain duck, series 30s upbeat bed at native 27.0s with the quiet close tail, no dynamic loudness. Captions burned for the two lines not already on cards (no-doubling rule); full sidecar `bemo-linkedin-sizzle-series-r4-30s.srt`. QA: 30.0s, -16.2 LUFS, -1.2 dBTP, speech rides ~15.7dB over the ducked bed. PENDING Becky: the cut itself, and whether the twin VO stands or she records the four lines (build re-runs unchanged either way: `capture/build-reel2.mjs`) |
| 8H-vH18 | 2026-08-09 | Becky's second tinny note, still on "Where missions gain momentum.", after vH17 fixed the swell. Measured root cause this time: `hg-r1-l8` is a splice, and its second sentence sits 4 LU thinner (low-vs-high tilt 6.0) than every neighboring read (9.8-10.1); the swell had been masking a thin source. Two fixes. (1) Fresh Avatar V read of the full close line on her twin (vH12 settings: "Bee Mo" phonetic, 1.12x, en-US), installed as `hgh-l8.wav` (lead-in trimmed, -18 LUFS pre-gain), tilt 9.8, matching the track; `hg-r1-l8.wav` stays as archive (reel 1's approved cuts still use it there). (2) A +2.5dB low shelf at 220Hz on the VO bus, after the compressor and identical for every segment, restoring the body the compressor shaves (l6-beat tilt back to 10.8 against vH13's 11.7; it had fallen to 8.7). Evenness recalibrates through the modified chain automatically (0.10 LU spread; the hot new read took -3.0dB). QA: 60.0s, -16.0 LUFS, -1.3 dBTP. Same frames as vH17. vH17 superseded. **APPROVED FINAL (Becky, 2026-08-09); the vH18 mix chain is the batch standard going forward, ratified into `docs/02-production-standards.md`** |
| 8H-vH17 | 2026-08-09 | Becky's two close notes. (1) The ground: the close sat on navy-deep (#01143A, the pressed inverse), which reads near-black on video; it moves to Deep Sapphire (#05347E), the canon's dark ground for the white knockout, which stays. (2) The audio ("off and tinny"): the music swell started at 52.2, under the spoken line. vH13's frame-leveler used to shove the VO above it by accident; the evened chain does not, so the voice sat ~1dB over a +3.8dB-hotter bed with the master limiter pinching hardest right there. SWELL_AT moves to 55.4: the bed holds 0.18 under the words (voice now ~5dB clear), and the swell rides the track's own final cadence, which ends naturally at 55.75 (the tail past 55.8 was always silent, in every version). QA: 60.0s, -16.0 LUFS, -0.9 dBTP; close frame verified Sapphire with the knockout. vH16 superseded |

...
const WORK = path.join(OUT, "build-letter");
...
const CAPTIONS = {
  cap3: "And then the letter existed.",
...
const LETTER = path.join(OUT, "draft", "page@ddd801c5068dc38361a5960df1ea61e9.webm"); // the drafted appeal
const CLOSEANIM = (() => {
...
beat(CONT, 9.5, 5, path.join(WORK, "cap1.png"), path.join(WORK, "s2.mp4"));
beat(BEAT2, 12.8, 5, path.join(WORK, "cap2.png"), path.join(WORK, "s3.mp4"));
beat(LETTER, 118, 4, path.join(WORK, "cap3.png"), path.join(WORK, "s3b.mp4"));
ff(["-loop","1","-t","6.5","-i",path.join(WORK,"qbase.png"),
    "-loop","1","-t","6.5","-i",path.join(WORK,"qa.png"),
    "-loop","1","-t","6.5","-i",path.join(WORK,"qb.png"),
    "-loop","1","-t","6.5","-i",MARK(),
    "-r",String(FPS),
    "-filter_complex",
    `[1:v]format=rgba,fade=t=in:st=0.3:d=0.6:alpha=1[qa];[2:v]format=rgba,fade=t=in:st=2.4:d=0.7:alpha=1[qb];[0:v][qa]overlay[x];[x][qb]overlay[y];[y][3:v]overlay=0:0,fade=t=out:st=5.9:d=0.6,format=yuv420p[o]`,
    "-map","[o]","-c:v","libx264","-crf","20","-preset","medium",path.join(WORK,"s4.mp4")]);
still(path.join(WORK, "card-category.png"), 3.0, path.join(WORK, "s5.mp4"));
...
fs.writeFileSync(list, ["s1", "s2", "s3", "s3b", "s4", "s5", "s6"].map((s) => `file '${path.join(WORK, s + ".mp4")}'`).join("\n"));

const FINAL = path.join(OUT, "bemo-amplify-feature-30s-v2-letter.mp4");

#!/usr/bin/env node
// Renders the motion-grammar audition of the press share cut: same script,
// same VO, same timings, same audio chain as run-press.mjs; only the motion
// grammar changes (see source-motion.html header). Audition, not a version.
//
//   node run-motion.mjs --build      splice wordmark into source-motion.built.html
//   node run-motion.mjs --render     record 2560x1440
//   node run-motion.mjs --finish     encode + VO + music + captions  (--outtag v1)
//
...
const SLUG = "common-table-press-release-motion";
// v4 (2026-08-10): the open names the work ("Drafting a press release never
// begins with the writing", pr-l2c.wav, regenerated same voice); app footage
// recut to one 16:10 window (seg3-*). Everything after l2 shifts +0.7s.
const DUR = 54.8, TAIL = 1.5;
const W = 2560, H = 1440;
...
const MIX = [
  [path.join(VO, "pr-l2c.wav"), 0.4],
  [path.join(VO, "pr-l3.wav"), 12.7],
  [path.join(VO, "pr-l4.wav"), 20.8],
  [path.join(VO, "pr-l5.wav"), 29.3],
  [path.join(VO, "pr-l6.wav"), 35.5],
  [path.join(VO, "pr-l7.wav"), 43.5],
  [path.join(VO, "pr-l8.wav"), 50.3],
];
...
const SWELL_AT = 49.7;
// tiny tick (the series chime's attack, sfx-tick.wav) under each chip/pill
// landing; texture, not a sound effect moment
const TICKS = [5.65, 7.4, 9.25, 22.7, 23.9, 25.3, 32.3, 33.7];
...
const CAPS = [
  [0.6, 3.2, "Drafting a press release never begins with the writing.", false],
  [3.7, 6.9, "It begins with the hunt: numbers in a spreadsheet,", true],
  [7.3, 11.7, "history in someone's head, boilerplate\\Nin whichever doc used it last.", true],
  [12.7, 18.3, "So here's one taking shape inside BeMo, where\\Nthe organization's knowledge already lives.", true],
  [20.8, 28.8, "Before anyone types a word, the boilerplate, the media\\Ncontact, the organization's own facts show up on their own.", true],
  [29.3, 34.7, "You answer the questions only a person can answer:\\Nwhat you're announcing, and why it matters.", true],
  [35.5, 42.5, "And the draft comes back sounding like the organization:\\Nits programs, its numbers, its own words.", true],
  [43.5, 49.5, "Every piece of content reflects who you actually are,\\Nnot who you were three drafts ago.", false],
  [50.3, 53.6, "BeMo. Where missions gain momentum.", false],
];
...
function build() {
  const tpl = fs.readFileSync(path.join(HERE, "source-motion.html"), "utf8");
  const b64 = (f, mime) =>
  ...
  fs.writeFileSync(path.join(HERE, "source-motion.built.html"), html);
  console.log("source-motion.built.html built.");
}
...
async function render() {
  ...
  await page.goto("file://" + path.join(HERE, "source-motion.built.html"));
...
async function finish() {
  ...
  fc += `;[${n + 2}:a]asplit=${TICKS.length}` + TICKS.map((_, i) => `[tk${i}]`).join("");
  fc += ";" + TICKS.map((t, i) =>

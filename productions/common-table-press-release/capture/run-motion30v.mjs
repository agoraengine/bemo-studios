...
//
//   node run-motion30.mjs --build      splice wordmark into source-motion30v.built.html
...
const SLUG = "common-table-press-release-motion30v";
...
function build() {
  const tpl = fs.readFileSync(path.join(HERE, "source-motion30v.html"), "utf8");
  const b64 = (f, mime) =>
  ...
  fs.writeFileSync(path.join(HERE, "source-motion30v.built.html"), html);
  console.log("source-motion30v.built.html built.");
}
...
async function render() {
  ...
  await page.goto("file://" + path.join(HERE, "source-motion30v.built.html"));
...
async function finish() {
  ...
  fc += `;[0:v]scale=1080:1920[vs]`;
  fc += `;[vs]subtitles='${burnSrt.replace(/'/g, "\\'")}':fontsdir='${path.join(HERE, "fonts").replace(/'/g, "\\'")}':force_style='FontName=Schibsted Grotesk,FontSize=9,PrimaryColour=&H003A2A1A,BorderStyle=4,BackColour=&H14FFFFFF,OutlineColour=&H14FFFFFF,Outline=1.1,Shadow=0,Alignment=2,MarginV=46'[vout]`;

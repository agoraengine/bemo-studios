...
- **The loft follows the presenter into every visible frame except the bubble** (Becky, 2026-08-04): split-screen segments render on the same loft scene as the full-screens, so the room stays continuous through the lesson. Only the small circular bubble uses the presenter's plain render; at that size the scene would be noise.

...
- **Full-bleed slides reserve the bubble zone** (Becky, 2026-08-06): slide content never enters the bottom-right where the circle lives (520px right padding in the deck's full layout; split-pane variants unaffected). The bubble can never cut off a graphic because nothing is ever placed under it.
- **Full-screen presenters sit back from the camera** (Becky, 2026-08-06): distance reads as real, close reads as avatar. Full-screen beats composite the transparent render at reduced scale (~0.85, bottom-anchored, `scale` in plan.json) over the scene, rather than HeyGen's full-frame placement.
